...
def logisticRegression(features, age, c=0.1, prediction=False, toPredict=np.empty(1, dtype=int)):
    # More info at :
    ...
    result = modelLogistic.fit(features, age)

    # We compute the score
    scoreFinal = modelLogistic.score(features, age)
    ...
    if prediction==True:
        ...
        return {'Coefficient': modelLogistic.coef_, 'C': c, 'Score': scoreFinal, 'Intercept': modelLogistic.intercept_, 'PredictedAges': predictionOutput}
    else:
...
for c in crange:
    ...
    results = logisticRegression(features, age, c, True, toPredict=toPredictFeatures)
    print("End of Logistic regression "+str(c))
